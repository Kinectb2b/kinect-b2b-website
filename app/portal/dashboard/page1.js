...
export default function ClientDashboard() {
  ...
  useEffect(() => {
    ...
    if (!clientData) { router.push('/portal/client/login'); return; }
  ...
  const createOrg = async () => {
    ...
    const { data, error } = await supabase.from('organizations').insert([{ name: newOrgName, industry: newOrgIndustry, website: newOrgWebsite, notes: newOrgNotes, client_id: client.id }]).select();
    if (!error && data) { 
      const newOrg = data[0];
      ...
      setNewOrgName(''); setNewOrgIndustry(''); setNewOrgWebsite(''); setNewOrgNotes(''); setShowAddOrg(false); 
    }
  };
  ...
  const updateOrg = async () => {
    await supabase.from('organizations').update({ name: selectedOrg.name, industry: selectedOrg.industry, website: selectedOrg.website, notes: selectedOrg.notes }).eq('id', selectedOrg.id);
    setOrganizations(organizations.map(o => o.id === selectedOrg.id ? selectedOrg : o)); setEditingOrg(false);
  };
  ...
  const createContact = async () => {
    ...
    const contactData = { name: newContactName, role: newContactRole, email: newContactEmail, phone: newContactPhone, organization_id: selectedOrg?.id, location_id: selectedLocation?.id || null };
    ...
    if (!error && data) {
      ...
      setNewContactName(''); setNewContactRole(''); setNewContactEmail(''); setNewContactPhone(''); setShowAddContact(false);
    }
  };
  ...
  const addActivity = async () => {
    ...
    if (!error && data) {
      ...
      setNewActivityType('note'); setNewActivityContent(''); setShowAddActivity(false);
    }
  };
  ...
  const handleLogout = () => { localStorage.removeItem('client'); sessionStorage.removeItem('client'); router.push('/portal/client/login'); };
            ...
            <div className="space-y-4">
                  ...
                  {showAddOrg && (
                    <div className="bg-white rounded-2xl p-6 border border-slate-200 mb-4">
                      ...
                      <div className="grid md:grid-cols-2 gap-4">
                        ...
                        <textarea placeholder="Notes" value={newOrgNotes} onChange={(e) => setNewOrgNotes(e.target.value)} className="px-4 py-2.5 border border-slate-200 rounded-xl" />
                      </div>
                      <div className="flex gap-3 mt-4"><button onClick={createOrg} className="px-6 py-2 bg-teal-600 text-white rounded-xl font-medium">Create</button><button onClick={cancelCreateOrg} className="px-6 py-2 bg-slate-200 rounded-xl">Cancel</button></div>
                    </div>
                  )}
              ...
              {selectedOrg && !selectedLocation && !selectedContact && (
                <div className="grid lg:grid-cols-3 gap-6">
                  <div className="lg:col-span-2 space-y-6">
                    <div className="bg-white rounded-2xl p-6 border border-slate-200">
                      <div className="flex justify-between items-start"><div className="flex items-center gap-4"><div className="w-16 h-16 bg-teal-100 rounded-2xl flex items-center justify-center text-teal-600 font-bold text-2xl">{selectedOrg.name?.charAt(0)}</div><div><h2 className="text-2xl font-bold text-slate-800">{selectedOrg.name}</h2>{selectedOrg.industry && <p className="text-slate-500">{selectedOrg.industry}</p>}</div></div><div className="flex gap-2"><button onClick={() => setEditingOrg(true)} className="px-4 py-2 bg-teal-600 text-white rounded-xl">Edit</button><button onClick={deleteOrg} className="px-4 py-2 bg-red-100 text-red-600 rounded-xl">Delete</button></div></div>
                    </div>
                    <div className="bg-white rounded-2xl p-6 border border-slate-200">
                      ...
                      {showAddContact && (
                        <div className="p-4 bg-teal-50 rounded-xl mb-4 space-y-3">
                          ...
                          <div className="grid grid-cols-2 gap-2">
                            ...
                            <input type="tel" placeholder="Phone" value={newContactPhone} onChange={(e) => setNewContactPhone(e.target.value)} className="px-3 py-2 border border-slate-200 rounded-lg" />
                          </div>
                          <div className="flex gap-2"><button onClick={createContact} className="flex-1 py-2 bg-teal-600 text-white rounded-lg">Add</button><button onClick={() => { setShowAddContact(false); setNewContactName(''); setNewContactRole(''); setNewContactEmail(''); setNewContactPhone(''); }} className="flex-1 py-2 bg-slate-200 rounded-lg">Cancel</button></div>
                        </div>
                      )}
                      {orgContacts.length === 0 ? <p className="text-slate-400 text-center py-4">No contacts</p> : (<div className="space-y-2">{orgContacts.map(contact => (<div key={contact.id} onClick={() => openContact(contact)} className="p-4 bg-slate-50 rounded-xl hover:bg-slate-100 cursor-pointer"><p className="font-medium text-slate-800">{contact.name}</p>{contact.role && <p className="text-sm text-slate-500">{contact.role}</p>}</div>))}</div>)}
                    </div>
                    <div className="bg-white rounded-2xl p-6 border border-slate-200">
                          ...
                          return (
                            <div key={deal.id} onClick={() => { setActiveTab('pipeline'); setTimeout(() => openDeal(deal), 100); }} className="p-4 bg-slate-50 rounded-xl hover:bg-slate-100 cursor-pointer">
                              <div className="flex justify-between items-start">
                                ...
                                <div className="text-right"><span className={`px-2 py-0.5 rounded-full text-xs ${stageInfo.bgLight} ${stageInfo.textColor}`}>{stageInfo.label}</span>{deal.deal_value > 0 && <p className="text-emerald-600 font-bold text-sm mt-1">${parseFloat(deal.deal_value).toLocaleString()}</p>}</div>
                              </div>
                            </div>
                          );
                        })}</div>
                  ...
                  <div>
                    <div className="bg-white rounded-2xl p-6 border border-slate-200">
                      ...
                      {orgActivities.length === 0 ? <p className="text-slate-400 text-center py-2 text-sm">No activity yet</p> : (
                        <div className="space-y-2 max-h-64 overflow-y-auto">
