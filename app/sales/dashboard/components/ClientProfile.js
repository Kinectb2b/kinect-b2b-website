...
export default function ClientProfile({ isOpen, onClose, client, currentUser, supabase, onUpdate, onCreateQuote, onAcceptQuote }) {
  ...
  const handleAcceptQuote = async () => {
    if (!clientData.quote_number) {
      alert('No quote found for this client');
      return;
    }
    ...
    try {
      ...
      const { data: quote, error } = await supabase
        .from('quotes')
        .select('*')
        .eq('quote_number', clientData.quote_number)
        .single();
  ...
  const handleCreateQuote = () => {
    // Pass client data to parent to open QuoteBuilder
    onCreateQuote(clientData);
    onClose();
  };
    ...
    <div className="fixed inset-0 bg-black/50 backdrop-blur-sm flex items-center justify-center z-50 p-4">
      <div className="bg-white rounded-2xl max-w-4xl w-full max-h-[90vh] overflow-hidden shadow-2xl">
        ...
        <div className="p-6 max-h-[calc(90vh-120px)] overflow-y-auto">
          {/* Status Badge and Create Quote Button */}
          <div className="mb-6 flex items-center justify-between flex-wrap gap-3">
            ...
            <div className="flex gap-2">
              {/* Accept Quote Button (only show if quote exists and not active) */}
              {clientData.quote_number && clientData.status !== 'active' && (
                <button
                  onClick={handleAcceptQuote}
                  className="px-6 py-2 bg-gradient-to-r from-green-600 to-emerald-600 text-white rounded-xl font-bold hover:from-green-700 hover:to-emerald-700 shadow-lg flex items-center gap-2"
                >
                  <span>✅</span>
              ...
              <button
                ...
                className="px-6 py-2 bg-gradient-to-r from-blue-600 to-indigo-600 text-white rounded-xl font-bold hover:from-blue-700 hover:to-indigo-700 shadow-lg flex items-center gap-2"
              >
                <span>💰</span>
          ...
          {/* Quote Number Display (if exists) */}
          {clientData.quote_number && (
            <div className="mb-6 bg-yellow-50 border-2 border-yellow-200 rounded-lg p-4">
              <p className="text-sm text-yellow-800">
                <span className="font-bold">Quote Sent:</span> {clientData.quote_number}
              </p>
            </div>
          )}

          {/* Active Services Display (if active) */}
          {clientData.status === 'active' && clientData.active_services?.length > 0 && (
            <div className="mb-6 bg-green-50 border-2 border-green-200 rounded-lg p-4">
              <p className="text-sm font-bold text-green-900 mb-2">Active Services:</p>
              <div className="flex flex-wrap gap-2">
                {clientData.active_services.map((service, idx) => (
                  <span key={idx} className="bg-green-200 text-green-800 px-3 py-1 rounded-full text-sm font-semibold">
                    {service}
                  </span>
                ))}
              </div>
          ...
          {/* Interested Services Display (for leads) */}
          {clientData.interested_services?.length > 0 && (
            <div className="mb-6 bg-blue-50 border-2 border-blue-200 rounded-lg p-4">
              <p className="text-sm font-bold text-blue-900 mb-2">Interested In:</p>
              <div className="flex flex-wrap gap-2">
                {clientData.interested_services.map((service, idx) => (
                  <span key={idx} className="bg-blue-200 text-blue-800 px-3 py-1 rounded-full text-sm font-semibold">
                    {service}
                  </span>
                ))}
              </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
                  ...
                  <div>
                    ...
                    <p className={`font-semibold ${
                      clientData.status === 'active' ? 'text-green-600' : 'text-blue-600'
                    }`}>
                      ${clientData.deal_value?.toLocaleString() || '0'}
                    </p>
                  </div>
                </div>
                {clientData.address && (
